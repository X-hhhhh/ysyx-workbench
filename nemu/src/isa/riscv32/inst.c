...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, s->dnpc = (src1 + imm) & 0xFFFFFFFE 
		 					IFDEF(CONFIG_FTRACE, ,Ftrace(s -> pc, s -> dnpc, 1, s -> isa.inst)));
  ...
  uint32_t t;
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, t = cpu.csr[imm], cpu.csr[imm] = t | src1, R(rd) = t);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, t = cpu.csr[imm], cpu.csr[imm] = src1, R(rd) = t);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm
		 					IFDEF(CONFIG_FTRACE, ,Ftrace(s -> pc, s -> dnpc, 0, s -> isa.inst)));
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = src1 - src2);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(rd) = src1 ^ src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (src2 != 0 && !(src2 == -1 && src1 == 0x80000000)) ? (int32_t)src1 % (int32_t)src2 : (src2 == 0) ? src1 : 0);
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (src2 != 0 && !(src2 == -1 && src1 == 0x80000000)) ? (int32_t)src1 / (int32_t)src2 : (src2 == 0) ? -1 : 0x80000000);
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, s->dnpc = cpu.csr[0x341]);	//set next pc to CSRS[mepc]
